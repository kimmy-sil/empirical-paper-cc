...
description: "文献检索与综述，含关键词策略、Proximity 评分、引用链追踪、防幻觉标记"
...
### 第二轮：引用链追踪（Proximity 1-2 文献必做）

对初筛中最相关的 3-5 篇核心论文，执行双向引用链追踪：

**向后追踪（Backward）**：核心论文引了谁？
- 目的：找到方法论源头和理论基础
- 重点关注：它的识别策略借鉴了哪篇？数据来源参考了谁？

**向前追踪（Forward）**：谁引了核心论文？
- 目的：找到最新进展和你的直接竞争者
- 工具：Google Scholar "被引用次数" 按时间排序
- 重点关注：最近 2 年引用了核心论文的实证研究

**输出**：将追踪发现的新文献加入筛选漏斗，标注"来源：[核心论文] 的引用链"。

### 第三轮：精读（方法+结论+贡献，30–60分钟/篇）
...
|------|------|
| 作者+年份 | |
| Proximity | 1-5（见下方评分标准） |
| 研究问题 | |
| 数据/样本 | |
| 识别方法 | |
| 核心发现 | |
| 与本文关系 | 支持/对比/方法借鉴 |
| 与本文的具体差异 | 数据/方法/样本/情境 |

### 第四轮：纳入决策（Proximity 评分）

为每篇文献打 Proximity 分（1 = 最近，5 = 最远）：

| 分数 | 含义 | 与本文关系 | 引用决策 |
|------|------|-----------|---------|
| 1 | 直接竞争 | 同问题 + 同方法/类似情境 | 必引，必须在贡献段说清差异 |
| 2 | 密切相关 | 同问题 + 不同方法或场景 | 必引，作为对比基准 |
| 3 | 相关 | 重叠主题 + 不同角度/结果 | 选引，视综述结构需要 |
| 4 | 背景 | 提供理论框架或方法基础 | 选引，用于 Background 节 |
| 5 | 边缘 | 仅提供一般性框架/语境 | 一般不引，除非综述需要 |

**关键规则：**
- Proximity 1-2 的文献 → 贡献定位段（第6节）必须逐篇说明本文与其差异
- Proximity 1 文献超过 3 篇 → 提醒用户：竞争激烈，需强化差异化

**最终文献库建议规模：** Proximity 1-2: 5-10篇；Proximity 3-4: 15-25篇；总引用 30-50条
...
**所有 LLM 生成的 BibTeX 条目必须标记 `% UNVERIFIED`：**

```bibtex
@article{smith2024innovation,  % UNVERIFIED
  author  = {Smith, John and Doe, Jane},
  title   = {Digital Innovation and Firm Productivity},
...
**规则：**
1. Claude 生成的任何 BibTeX 条目 → 行尾加 `% UNVERIFIED`
2. 用户手动确认或从 Zotero/Scholar 验证后 → 移除标记
3. `% UNVERIFIED` 条目禁止出现在最终提交稿中
4. paper-writing skill 引用时，遇到 UNVERIFIED 条目 → 自动标记 `\textbf{[VERIFY]}`

**禁止行为：**
- ❌ 编造不存在的论文（零容忍）
- ❌ 猜测 volume/pages/doi（宁可留空，不可编造）
- ❌ 张冠李戴（把 A 的发现归给 B）

**允许行为：**
- ✅ 提供论文标题 + 作者 + 年份，标记 `% UNVERIFIED`，volume/pages 留空
- ✅ 建议用户去 Scholar 验证并补全信息
...
**Proximity 1-2 的文献必须在贡献段逐篇点名**（不可笼统写"已有文献"）。
